...
#define MAXRLEN 18 	//最大长度

static uint8_t Rc522_LinkFlag;

uint8_t CT[2];		// 卡类型
uint8_t SN[4];		// 卡号
uint8_t READ_RFID[16];		// 存放RFID读取的数据
uint8_t WRITE_RFID[16]={"/-A*9xBC|<D2mE-/"};	// 存放RFID写入的数据
...
{
	RC522_Uart_init(9600);	// 串口1初始化为9600，未开启中断
	delay_ms(500);
	InitRc522();			//读卡器初始化
}
