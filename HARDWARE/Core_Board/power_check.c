...
uint8_t Electric_Buf[2];
float Pa =0.0,Pb =0.0;  // 计算参数

uint8_t pwr_ck_l =0;  // 电量数据
uint8_t pwr_ck_r =0;
...
{ 	
	...
	ADC_RegularChannelConfig(ADC1,ADC_Channel_6,1,ADC_SampleTime_144Cycles);	//设置指定ADC的规则组通道，设置它们的转化顺序和采样时间
	ADC_Cmd(ADC1,ENABLE);
}
...
{
	ADC_ClearFlag(ADC1,ADC_FLAG_EOC);	//清除ADC1的待处理标志位(参数1--ADC1 参数2--转换结束标志位)
	ADC_SoftwareStartConv(ADC1);	//启用选定的 ADC 软件启动常规通道的转换
	while(!ADC_GetFlagStatus(ADC1, ADC_FLAG_EOC));	//循环等待直到返回值为SET才退出
...
{
	temp =  Get_Electricity(POWER_CHECK_NUM);	//平均值滤波
	temp =  Smoothing_Filtering(temp);	//平滑滤波
	temp_value = MLib_GetSub(temp,temp2);	//作差
	if(temp2 == 0)
	{
			temp = (Pa*temp); // 电量计算方法		
