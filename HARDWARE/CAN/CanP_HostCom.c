			...
			if(CanP_RxFMI_Flag & bit_tab[i])
			{
				...
				case 0:
					while(FifoDrv_ReadOne(&Fifo_Info,&t8));
						break;
				case 1:		//这里应该是wifi接收判断			
					while(FifoDrv_ReadOne(&Fifo_WifiRx,&t8))
					{
						Can_WifiRx_Save(t8);
					}
					break;
				case 2:	//这里应该是zigbee接收判断	
		...
		for(j = 0;j<=4;j++)
		{
			switch(m)
			{
				case 0:
					...
					break;
				case 1:
					i = FifoDrv_BufRead(&Fifo_WifiTx,ctbuf,8);
					if(i)
					{
						f = 1;
						CanDrv_TxData(ctbuf,i,CAN_SID_HL(ID_WIFI,0),0,&tmbox);
					...
					break;
				case 2:
					i = FifoDrv_BufRead(&Fifo_ZigbTx,ctbuf,8);
					if(i)
					{
						f = 1;
						CanDrv_TxData(ctbuf,i,CAN_SID_HL(ID_ZIGBEE,0),0,&tmbox);
					...
					break;
				case 3:
					i = FifoDrv_BufRead(&Fifo_Info,ctbuf,8);
					if(i)
					{
						f  = 1;
						CanDrv_TxData(ctbuf,i,CAN_SID_HL(ID_DISP,0),0,&tmbox);
...
返 回 值：无
*/
void Send_Electric(uint8_t x1, uint8_t x2)  
{
	uint8_t txbuf[3];
//	txbuf[0] =0x01;
//	txbuf[1] = x1;
...
void Send_CodedCnt(void)
{
	...
	CanP_Cmd_Write(CANP_CMD_ID_CNT,txbuf,5,CAN_SID_HL(ID_HOST,0),0);
}
...
返 回 值：无
*/
//void Send_Debug_Info(uint8_t *s,uint8_t len)
//{	  
//	CanDrv_WhaitTxEmpty();
//	CanDrv_TxData( s,len,CAN_SID_HL(ID_DISP,0),0,_NULL);	
//}

//void Send_ZigBee_Info(uint8_t *s,uint8_t len)
//{
//	CanDrv_WhaitTxEmpty();
...
void Send_UpCompass(uint16_t c)
{
	...
	CanP_Cmd_Write(CANP_CMD_ID_NV,tbuf,2,CAN_SID_HL(ID_NAVIG,ID_NAVIG),0);
}
...
返 回 值: 无
*/
void Host_Receive_UpTrack( uint8_t x1, uint8_t x2)  // 循迹数据接收处理函数
{
	Track_buf[0] = x1;
	Track_buf[1] = x2;	 
}
...
返 回 值：Rt 循迹数据
*/
uint16_t  Get_Host_UpTrack( uint8_t mode)  // 获取循迹数据
{
	uint16_t Rt = 0;
	switch(mode)
	{
...
返 回 值：无
*/
void Set_Track_Pwr( uint16_t power) // 设置寻迹板发射功率
{
	uint8_t txbuf[3];
	...
	CanP_Cmd_Write(CANP_CMD_ID_T0,txbuf,3,CAN_SID_HL(ID_TRACK,ID_HOST),0);
}
...
返 回 值：无
*/
void Set_Track_Yzbj(uint8_t addr, uint16_t ydata)
{
	uint8_t txbuf[4];
...
返 回 值: 无
*/
void Host_Set_UpTrack( uint8_t time)  // 设置循迹数据上传时间间隔
{
#if 1
	uint8_t txbuf[2];
	txbuf[0] = 0X02;  //命令关键字
	...
	CanP_Cmd_Write(CANP_CMD_ID_T2,txbuf,2,CAN_SID_HL(ID_TRACK,ID_HOST),0);
#endif
	
}

/**
函数功能：将数据发送至ZigBee模块，在ZigBee区显示
参    数：无
返 回 值：无
*/
...
/**
函数功能：将数据发送至WIFI模块，在WiFi区显示
参    数：无
返 回 值：无
*/
void Send_WifiData_To_Fifo( uint8_t *p ,uint8_t len)   
{
...
*/
void Send_InfoData_To_Fifo(char *p ,uint8_t len)
{
	FifoDrv_BufWrite(&Fifo_Info, (uint8_t *)p, len);
	delay_ms(100);
}
...
{
	uint8_t i;
	for(i = 0;i<CANP_CMD_ID_SIZE;i++)
		CanP_Cmd_SBuf[i].l = 0;
	
}

void CanP_Cmd_Write(uint8_t id,uint8_t *p,uint8_t l,uint32_t sid,uint32_t eid)
{
	if(id < CANP_CMD_ID_SIZE)
	{
		CanP_Cmd_SBuf[id].l = l;
		memcpy(CanP_Cmd_SBuf[id].d,p,l);
		CanP_Cmd_SBuf[id].sid = sid;
		CanP_Cmd_SBuf[id].eid = eid;
	}
}

int8_t CanP_Cmd_Check(void)
{
	int8_t Rt = -1;
	uint8_t i;
	for(i = 0;i<CANP_CMD_ID_SIZE;i++)
	{
		if(CanP_Cmd_SBuf[i].l)
		{
			Rt = i;
			break;
		}
...
void CanP_Cmd_Send(uint8_t id)
{
#define SP CanP_Cmd_SBuf[id]
	CanDrv_TxData(SP.d,SP.l,SP.sid,SP.eid,_NULL);
	SP.l = 0;
#undef SP
}
