...
void LED_Dis_Zigbee(uint16_t dis)
{
    ...
    Zigbee[2] = 0x04;
    Zigbee[3] = 0x00;
    Zigbee[4] = dis / 100 % 10;	//获取高位
    Zigbee[5] = ((dis / 10 % 10) * 16 + dis % 10);	//剩余低位
    Zigbee[6] = (Zigbee[2] + Zigbee[3] + Zigbee[4] + Zigbee[5]) % 256;
    Zigbee[7] = 0xBB;
    Send_ZigbeeData_To_Fifo(Zigbee, 8);  //发送Zigbee数据
    delay_ms(100);
...
void Track_duoy_RFID(uint8_t speed)   //寻卡寻迹 
{
#if 0	//自己加
	Roadway_mp_syn();       // 码盘同步
#endif
...
void Track_duo_RFID(uint8_t speed,uint8_t card)//读卡函数   参数：循迹速度，需要读取的数据块
{
